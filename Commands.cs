...
namespace MetaphysicsIndustries.Ligra
{
    public static class Commands
    {
        ...
        public static void Example2Command(string input, string[] args, LigraEnvironment env)
        {
            ...
            expr = parser.GetExpression("unitstep((x*x+y*y)^0.5+2*(sin(t)-1))*cos(5*y+2*t)", env);
            env.RenderItems.Add(new InfoItem("unitstep((x*x+y*y)^0.5+2*(sin(t)-1))*cos(5*y+2*t)", env.Font));
            env.RenderItems.Add(new Graph3dItem(expr, Pens.Black, Brushes.Green, -4, 4, -4, 4, -2, 6, "x", "y"));
...
//            var func = new UserDefinedFunction(funcname, argnames, expr);
        ...
        public static void OpenTkCommand(string input, string[] args, LigraEnvironment env)
        {
            if (!env.Variables.ContainsKey("x")) env.Variables.Add("x", new Literal(0));
            if (!env.Variables.ContainsKey("y")) env.Variables.Add("y", new Literal(0));
            ...
            var parser = new LigraParser();
            var formula = "unitstep((x*x+y*y)^0.5+2*(sin(t)-1))*cos(5*y+2*t)";
            expr = parser.GetExpression(formula, env);
            env.RenderItems.Add(new InfoItem(formula, env.Font));
            env.RenderItems.Add(new OpenTk3dItem(expr, Pens.Black, Brushes.Green, -4, 4, -4, 4, -2, 6, "x", "y", env.Font));
